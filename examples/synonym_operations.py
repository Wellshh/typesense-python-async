...
create_synonym_response = client.collections["books"].synonyms.upsert(
    "hp-philosopher-sorcerer", {"synonyms": ["philosophers", "sorcerers"]}
)
...
print(
    client.collections["books"].documents.search(
        {"q": "sorcerers", "query_by": "title", "sort_by": "ratings_count:desc"}
    )
)

# Update synonym: convert to 1-way synonym
update_synonym_response = client.collections["books"].synonyms.upsert(
    "hp-philosopher-sorcerer", {"root": "philosophers", "synonyms": ["sorcerers"]}
)
